...
@itemlist[
  ...
  @item{Normal distribution quantifiers for realistic length variation}
...
@itemlist[
  @item{@litchar{{n}} - Exactly n repetitions}
  @item{@litchar{{n+}} - Normal distribution with mean n (2nd order)}
  @item{@litchar{{n++}} - Normal distribution with mean n (3rd order, more concentrated)}
  @item{@litchar{{n1+n2}} - Normal distribution in range n1..n2 (2nd order)}
  @item{@litchar{{n1++n2}} - Normal distribution in range n1..n2 (3rd order)}
  @item{@litchar{{+n}} - Shorthand for @litchar{{0+n}} (range 0..n)}
  @item{@litchar{{++n}} - Shorthand for @litchar{{0++n}} (range 0..n, 3rd order)}
  @item{@litchar{(?<name>...)} - Named group (captures pattern for later reference)}
  @item{@litchar{\\k<name>} - Backreference to named group}
...
@section{Normal Distribution Quantifiers}
...
Higher order (more @litchar{+} signs) means values are more concentrated around the center/mean.

@section{Named Groups and Backreferences}
...
Named groups are defined with @litchar{(?<name>...)} and referenced with @litchar{\\k<name>}.
The backreference will produce the exact same string that was generated by the named group.
...
@itemlist[
  @item{New: Normal distribution quantifiers (@litchar{{n+}}, @litchar{{n++}}, etc.) for realistic length variation}
  @item{New: Range normal distribution (@litchar{{n1+n2}}, @litchar{{n1++n2}}, @litchar{{+n}}, @litchar{{++n}})}
  @item{New: Named groups @litchar{(?<name>...)} for capturing generated content}
  @item{New: Backreferences @litchar{\\k<name>} for reusing captured content}
]
