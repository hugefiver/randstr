...
(define (check pattern)
  (case (substring pattern 0 (min 2 (string-length pattern)))
    [("\\p")
     ;; For Unicode property patterns, we'll use direct validation
     (let* ([tokens (tokenize-pattern pattern)]
            [generated (generate-from-tokens tokens)]
            [char (string-ref generated 0)])
       (case (substring pattern 3 (- (string-length pattern) 1))  ; extract property name from \p{...}
...
(test-case "generate-from-tokens: block Unicode properties"
  ...
  (check-true (check-unicode-property-generate "\\p{Block=Latin_1_Supplement}" "Block=Latin_1_Supplement"))
